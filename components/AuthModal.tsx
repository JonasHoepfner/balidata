...
async function redirectAfterPasswordLogin() {
...
function PasswordForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isSignUp) {
      ...
      setLoading(false)
      if (authError) { setError(translateAuthError(authError.message)); return }
      onSuccess ? onSuccess() : await redirectAfterPasswordLogin()
    }
  }
...
export default function AuthModal() {
  ...
  async function handleGoogle() {
    ...
    const supabase = createSupabaseBrowserClient()
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL ?? 'http://localhost:3000'
    await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: baseUrl + '/auth/callback' } })
  }

  if (!isOpen) return null

  return (
    <div onClick={handleClose} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.82)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 440, background: '#111111', border: '1px solid #2A2A2A', borderRadius: 12, padding: '40px 36px', position: 'relative' }}>

        <button onClick={handleClose} style={{ position: 'absolute', top: 16, right: 18, background: 'none', border: 'none', color: '#4A4540', fontSize: 22, cursor: 'pointer', lineHeight: 1, padding: '2px 6px' }}>×</button>

        <div style={{ textAlign: 'center', fontFamily: 'var(--font-dm-mono)', fontSize: 14, color: '#C4A882', letterSpacing: '0.14em', marginBottom: 20 }}>BALIDATA</div>

        <h2 style={{ fontFamily: 'var(--font-cormorant)', fontSize: 32, fontWeight: 600, color: '#F0EAE2', textAlign: 'center', letterSpacing: '-0.02em', marginBottom: 8, lineHeight: 1.1 }}>
          Accédez à votre analyse
        </h2>
        <p style={{ fontFamily: 'var(--font-outfit)', fontSize: 14, color: '#6A6158', textAlign: 'center', marginBottom: 28, lineHeight: 1.6 }}>
          Connectez-vous pour recevoir votre rapport complet.
        </p>

        {/* Google */}
        <button onClick={handleGoogle} disabled={googleLoading} style={{ width: '100%', padding: '12px 16px', borderRadius: 50, border: '1px solid #D1D5DB', background: googleLoading ? '#F3F4F6' : '#FFFFFF', color: '#1F2937', fontSize: 14, fontWeight: 600, fontFamily: 'var(--font-outfit)', cursor: googleLoading ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, opacity: googleLoading ? 0.7 : 1 }}>
          <GoogleIcon />
        ...
        {mode === 'magic' ? (
          ...
          <PasswordForm onSuccess={async () => { handleClose(); await redirectAfterPasswordLogin() }} />
        )}

        <p style={{ textAlign: 'center', marginTop: 20, fontFamily: 'var(--font-dm-mono)', fontSize: 9, color: '#2A2520', letterSpacing: '0.06em' }}>
